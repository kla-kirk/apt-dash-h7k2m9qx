/* AMENITIES layer module — owned by the "amenities" chat. Data: amenities.json = [[name,type,lat,lon]].
   Per-listing detail (BRMap.addDetail): on listing select, draws the nearest of each type as bold
   color-coded CIRCLE markers + bold name labels (pane-labels) joined to the home by a same-colored
   connector (pane-connect). MULTIPLE closest are shown for some types: grocery top-3, park top-2, all
   others nearest-1 (NCFG). The nearest is full-size; further options are smaller/lighter ("alt").
   Click an amenity (its map circle OR its row in the listing #info panel) → opens its detail card
   (name, category, distance, approx walk/drive, Directions) AND draws the street route home↔amenity via
   the commute module's window.BRCommute.drawAmenityRoute (the route only appears where commute has
   generated one — currently the nearest-1 of each type; commute regenerates for top-N separately).
   The card is a Leaflet tooltip (not a popup) and its clicks stopPropagation so the listing stays
   selected. Panel rows are built via addPopupRow and wired through BRMap.onDetailRender. */
BRMap.ready(async () => {
  const map = BRMap.map;
  ...
  const NCFG  = { grocery: { n: 3, r: 2.5 }, park: { n: 2, r: 2.5 } };       // count + scan-radius (mi) per type
  const cfg = (t) => NCFG[t] || { n: 1, r: 1.6 };                            // everything else: nearest-1 within 1.6mi
  ...
  // { type: [ {a,d}, ... up to cfg(type).n, sorted nearest-first ] }
  ...
  function showCard(a, d, l) { const key = a[0] + "@" + a[2] + "," + a[3];
    if (cardKey === key) { hideCard(); return false; }
    hideCard();
  ...
  // open a card AND draw the street route home↔amenity (commute module owns the route geometry)
  function focusAmenity(a, d, l) { const opened = showCard(a, d, l);
    try { if (window.BRCommute) { if (opened && BRCommute.drawAmenityRoute) BRCommute.drawAmenityRoute(l.address, a[0]);
      else if (!opened && BRCommute.clearAmenityRoute) BRCommute.clearAmenityRoute(); } } catch (e) {} }
